...
/**
 * 웹 `shared/ui/Textarea` 의 RN 미러.
 *
 * 웹과 동일하게 FormProvider 안에서만 동작한다 — 화면은 name 만 넘긴다.
 * Controller 를 화면이 아니라 이 안에 두는 것이 웹과 같은 계약을 만든다.
 */
const Textarea = <T extends FieldValues>(props: CustomTextAreaPropsType<T>) => {
  const {
    name,
    label,
    button,
          ...
          <View className="w-full">
            ...
            <View className="relative">
              <TextInput
                ...
                className={[
                  'block h-28 w-full rounded-md border border-[#f8f8f8] px-3 py-2 text-grayscale900',
