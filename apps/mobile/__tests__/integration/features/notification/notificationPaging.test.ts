 ...
 * 여기서도 그렇게 흉내 내야 0-based 로 요청했을 때 생기는 중복이 드러난다.
 */
  ...
  server.use(
    http.get(`${BASE}/members/me/notifications`, ({ request }) => {
      ...
      return HttpResponse.json({
        status: 200,
        success: true,
        data: { size: 10, page: Math.max(page, 1), totalPages: 1, totalCount: 10, resultsSize: results.length, results },
      });
    }),
  );
};
...
describe('알림이 딱 한 페이지를 채울 때', () => {
  it('다음 페이지를 더 받아도 같은 알림이 두 번 보이지 않는다', async () => {
    ...
    // 10건이 꽉 찼으니 목록은 다음 페이지가 있다고 판단한다 — 화면에서 끝까지 스크롤한 상황이다.
    ...
    // 목록이 같은 알림을 두 번 그리면 SectionList 의 키가 겹쳐 항목이 사라지거나 중복된다.
